...
namespace LaravelQueue\Nsq\Lookup;

use Illuminate\Support\Facades\Log;
use LaravelQueue\Nsq\Exception\LookupException;
...
class Lookup
{
     ...
     */
    public function __construct(array $hosts = [], float $connectionTimeout = 1.0, float $responseTimeout = 2.0)
    {
        if (empty($hosts)) {
            $this->hosts = [
     ...
     * @return array
     */
    public function lookupHosts(string $topic): array
    {
        $lookupHosts = [];
        $topicChannel = [];
